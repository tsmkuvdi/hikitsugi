...
try {
	...
	$id_hikitsugi = (int) $_POST['id'];
	$dbh = new PDO("mysql:host=localhost;dbname=$databasename;charset=utf8", $user, $pass);
	$dbh->setAttribute(PDO::ATTR_EMULATE_PREPARES, false);
	$dbh->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	$sql = "UPDATE $dbtablename SET hizuke = ?, naiyou = ?, tantou = ?,category = ?, shinkou = ? WHERE id_hikitsugi = ?";  //主キーは最後にWHERE
	...
	$stmt->bindValue(6, $id_hikitsugi, PDO::PARAM_INT);
	$stmt->execute();
	$dbh = null;
	echo "ID: " . htmlspecialchars($id_hikitsugi,ENT_QUOTES,'UTF-8') ."引継内容の更新が完了しました。<br>";
echo "<br>";
